...
class TestStreamingCoverage(unittest.TestCase):
    ...
    def test_final_buffer_yield_line_65_guaranteed(self):
        """Test final buffer yield when stream ends with remaining content (line 65)."""

        def chunk_stream():
            # Send content that will remain in buffer at end without any placeholders
            yield "This content stays in buffer"
            # No more chunks - this should trigger the final: if buffer: yield buffer
    ...
    def test_chunk_processing_comprehensive(self, description, chunks, expected):
    ...
    def test_buffer_accumulation_and_final_yield_line_65(self):
        """Test buffer accumulation with final yield - absolutely guaranteed to hit line 65."""

        def partial_placeholder_stream():
            # Start a placeholder but don't complete it, leaving content in buffer
            yield "<PERSON"
            yield "_0>"
    ...
    def test_empty_buffer_edge_case_line_65(self):
        ...
        result = list(uncloak_stream_response(empty_chunk_stream(), self.entity_map))

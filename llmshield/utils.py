...
from typing import Any, BinaryIO, Generator, Protocol, runtime_checkable
...
def wrap_entity(
    entity_type: EntityType, suffix: int, start_delimiter: str, end_delimiter: str
) -> str:
    """
...
def ask_helper(
    ...
    This function checks if the input should be cloaked and handles both streaming and non-streaming cases.
    """
    ...
    if _should_cloak_input(input_data=input_text):
        ...
        func_preferred_param = (
            "message"
            if "message" in shield._llm_func.__code__.co_varnames
            else "prompt"
        )
